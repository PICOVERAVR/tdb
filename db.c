...
const char *help_text = "Help menu: \n\
    ...
    start/continue (c): start or resume execution of program \n\
    break (b) <hex addr>: stop execution at addr \n\
    info (i): dumps some helpful register values \n\
		...
		while (!running) {
			...
			switch (temp) {
				...
				case B_CONTINUE:
					...
					break;
				case B_BREAK:
					if (bp_free == NUM_BREAKPOINTS) {
						printf("ERR: no more space for breakpoints.\n");
					}
